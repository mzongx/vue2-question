    	...
    	<div v-if="fatherComponent == 'item'">
    		<div class="item_back item_container_style">
    			<div class="item_list_container">
    				...
    				<ul>
    					<li class="item_list" v-for="(item, index) in itemDetail[itemNum-1].topic_answer" @click="choosed(index,item.topic_answer_id,item.is_standard_answer)">
...
export default {
  	...
  	methods: {
	  	...
	  	nextitem() {
	  		if (this.choosedNum !== null) {
	  			...
	  			this.$store.dispatch('addNum',{id:this.choosedId, res:this.isRight})
	...
	computed: {
		//...是es6的解构运算符，也就是展开对象数组的属性,以下就是获取store中的state，因为计算机属性名跟state节点名一样，所以可以简化一个数组
		...mapState([
			'itemNum','level','itemDetail'
		])
	},
	created(){
